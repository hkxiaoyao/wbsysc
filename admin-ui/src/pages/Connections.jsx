...
export default function Connections({ tenantId = '', initialConnectionId = '', embedded = false, onViewLogs = () => {} }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const responses = await Promise.all(tenantIds.map((id) => api.get(`/admin/tenants/${encodeURIComponent(id)}/connections`)))
      ...
      const enriched = await Promise.all(listed.map(async (connection) => {
        ...
        const [detailResponse, toolResponse] = await Promise.all([
          api.get(`/admin/connections/${id}`).catch(() => null),
          api.get(`/admin/connections/${id}/tools`).catch(() => null),
        ])
        const token = detailResponse?.data?.tokens?.[0]
        return {
          ...connection,
          token_prefix: token?.prefix || token?.token_prefix || '',
  ...
  useEffect(() => { load() }, [scopeTenant])
  useEffect(() => () => setTokenModal(closeTokenModal()), [])
  ...
  const openDetail = useCallback(async (row) => {
    ...
    try {
      const [detailResponse, toolResponse] = await Promise.all([
        api.get(`/admin/connections/${encodeURIComponent(row.connection_id)}`),
        api.get(`/admin/connections/${encodeURIComponent(row.connection_id)}/tools`).catch(() => ({ data: { items: [] } })),
      ])
    ...
    } catch (error) { messageApi.error(safeServerError(error, '连接详情加载失败')) }
    finally { setDrawerBusy('') }
  }, [messageApi])

  useEffect(() => {
    if (initialConnectionId && items.length && !detail) {
      const target = items.find((item) => item.connection_id === initialConnectionId)
      if (target) openDetail(target)
    }
  }, [initialConnectionId, items, detail, openDetail])
  ...
  const showToken = (connection, rawToken) => setTokenModal({
    open: true, rawToken, connectionId: connection.connection_id,
  })
  const dismissToken = () => setTokenModal(closeTokenModal())

  const create = async () => {
    setCreateBusy(true)
    try {
      const values = await form.validateFields()
      const declarative = values.connector_key === 'http_declarative'
  ...
  const saveTools = async () => {
    const invalidWrite = tools.some((tool) => tool.enabled && !canEnableWriteTool(tool, {
      explicitEnable: true, explicitWrite: tool.policy?.allow_write,
    }))
  ...
  const tabs = detail ? [
    {
      key: 'config', label: '配置', children: <div className="connection-editor"><Alert type="info" showIcon message="公开配置不会包含凭据或 Token" /><label>连接名称<Input value={detail.display_name} onChange={(event) => setDetail((current) => ({ ...current, display_name: event.target.value }))} /></label><label>数据模式<Select value={detail.data_mode} options={Object.entries(DATA_MODE).map(([value, label]) => ({ value, label }))} onChange={(data_mode) => setDetail((current) => ({ ...current, data_mode }))} /></label><label>公开配置（JSON）<Input.TextArea rows={9} value={configText} onChange={(event) => setConfigText(event.target.value)} /></label><Button type="primary" loading={drawerBusy === 'config'} onClick={saveConfig}>保存配置</Button></div>,
    },
    {
      key: 'credentials', label: '凭据', children: <div className="connection-editor"><Alert type="warning" showIcon message="只输入一组完整的新凭据" description="现有值永不返回浏览器。保存或关闭工作台后，输入内容会立即清除。" /><Input.TextArea rows={8} value={credentialsText} onChange={(event) => setCredentialsText(event.target.value)} placeholder='{"api_key":"new value"}' /><Button type="primary" loading={drawerBusy === 'credentials'} onClick={saveCredentials}>替换凭据并清除输入</Button></div>,
    },
    {
      key: 'tokens', label: `Token · ${tokens.length}`, children: <div className="connection-editor"><Alert type="info" showIcon message="Token 原文只在签发后显示一次" /><Input maxLength={128} value={tokenLabel} placeholder="用途标签（可选）" onChange={(event) => setTokenLabel(event.target.value)} /><Space><Button loading={drawerBusy === 'token'} onClick={() => issueToken(false)}>签发新 Token</Button><Button danger loading={drawerBusy === 'token'} onClick={() => issueToken(true)}>轮换并撤销旧 Token</Button></Space><div className="connection-token-list">{tokens.map((token) => <div key={token.token_id}><Text code>{token.prefix || token.token_prefix || 'token'}</Text><Text type="secondary">{token.label || '未命名'}</Text><Button danger type="link" size="small" onClick={() => revokeToken(token)}>撤销</Button></div>)}</div></div>,
    },
    {
      key: 'tools', label: `工具 · ${tools.length}`, children: <div className="connection-editor"><Alert type="warning" showIcon message="写操作需要双重确认" /><div className="connection-tools">{tools.map((tool, index) => <div key={tool.tool_key}><div><Text code>{tool.mcp_name || tool.tool_key}</Text><Tag color={tool.operation_kind === 'write' ? 'orange' : 'blue'}>{tool.operation_kind}</Tag></div><Space wrap><Switch checked={Boolean(tool.enabled)} checkedChildren="启用" unCheckedChildren="停用" onChange={(enabled) => setTools((current) => current.map((item, i) => i === index ? { ...item, enabled } : item))} />{tool.operation_kind === 'write' && <Switch checked={Boolean(tool.policy?.allow_write)} disabled={!tool.enabled} checkedChildren="同意写入" unCheckedChildren="未授权写入" onChange={(allow_write) => setTools((current) => current.map((item, i) => i === index ? { ...item, policy: { ...(item.policy || {}), allow_write } } : item))} />}</Space></div>)}</div><Button type="primary" loading={drawerBusy === 'tools'} onClick={saveTools}>保存工具策略</Button></div>,
    },
    {
    ...
    ...(detail.connector_key === 'http_declarative' ? [{ key: 'wizard', label: '声明式向导', children: <DeclarativeSpecWizard connection={detail} onChanged={(next) => { if (next) setDetail(next); load() }} /> }] : []),
  ] : []
    ...
    <main className={`connection-workbench ${embedded ? 'connection-workbench--embedded' : ''}`}>
      ...
      <section className="connection-panel">
        <div className="connection-toolbar">
          {!tenantId && <Select allowClear showSearch optionFilterProp="label" placeholder="全部租户" value={scopeTenant || undefined} options={tenants.map((tenant) => ({ value: tenant.tenant_id, label: tenant.display_name ? `${tenant.display_name} · ${tenant.tenant_id}` : tenant.tenant_id }))} onChange={(value) => setScopeTenant(value || '')} />}
          <Input allowClear value={query} placeholder="搜索名称、连接 ID 或租户" onChange={(event) => setQuery(event.target.value)} />
          <Select value={connectorFilter} options={[{ value: 'all', label: '全部连接器' }, ...[...new Set(items.map((item) => item.connector_key))].map((value) => ({ value, label: value }))]} onChange={setConnectorFilter} />
          <Select value={statusFilter} options={[{ value: 'all', label: '全部状态' }, ...Object.entries(STATUS_META).map(([value, meta]) => ({ value, label: meta.label }))]} onChange={setStatusFilter} />
      ...
      <Drawer rootClassName="connection-drawer" title={detail ? `${detail.display_name} · 连接工作台` : '连接工作台'} open={Boolean(detail)} onClose={closeDetail} width={screens.lg ? 960 : screens.sm ? '92vw' : '100vw'} loading={drawerBusy === 'detail'} extra={detail && <Tag color={(STATUS_META[detail.status] || STATUS_META.disabled).color}>{(STATUS_META[detail.status] || STATUS_META.disabled).label}</Tag>}>
        {detail && <><div className="connection-drawer-rail"><span><ApiOutlined /> {detail.connector_key}</span><span>{DATA_MODE[detail.data_mode] || detail.data_mode}</span><Text code>{endpoint(detail.connection_id)}</Text></div><Tabs items={tabs} onChange={(key) => { if (key !== 'credentials') setCredentialsText('{}') }} /></>}
      </Drawer>
      ...
      <Modal title="新建连接实例" open={createOpen} onCancel={() => { setCreateOpen(false); form.resetFields() }} onOk={create} confirmLoading={createBusy} okText="创建连接" cancelText="取消" width={620}>
        <Form form={form} layout="vertical" initialValues={{ tenant_id: tenantId || scopeTenant || undefined, connector_key: 'wecom', data_mode: 'direct', public_config: '{}', credentials: '{}' }}>
          ...
          <Form.Item name="connector_key" label="连接器" rules={[{ required: true }]}><Select options={[{ value: 'wecom', label: '企业微信（wecom）' }, { value: 'http_declarative', label: 'HTTP 声明式（导入 JSON/YAML）' }]} /></Form.Item>
          <Form.Item name="data_mode" label="数据模式" rules={[{ required: true }]}><Select options={Object.entries(DATA_MODE).map(([value, label]) => ({ value, label }))} /></Form.Item>
          <Form.Item noStyle shouldUpdate={(previous, current) => previous.connector_key !== current.connector_key}>{({ getFieldValue }) => getFieldValue('connector_key') === 'http_declarative' ? <Alert type="info" showIcon message="连接将以草稿创建" description="创建后进入声明式向导导入 JSON/YAML；这里不会接受代码、模板或预置凭据。" /> : <><Form.Item name="public_config" label="公开配置（JSON 对象）"><Input.TextArea rows={5} /></Form.Item><Form.Item name="credentials" label="初始凭据（JSON 对象）"><Input.TextArea rows={5} /></Form.Item></>}</Form.Item>
        </Form>
      </Modal>
