...
- **开放授权（OAuth）**：登记精确重定向 URI，禁止前缀和通配匹配，校验 `state`，凭证只写加密存储
...
主线程在取得浏览器和 MySQL 环境后执行本节。使用非生产 ID 和唯一前缀，例如 `smoke_1234567890123`，不要在本任务中连接外部环境。
...
优先使用管理 API 删除测试资源。若 API 无法完成清理，请在备份后执行以下 MySQL 5.7 事务，并替换描述性占位值：
